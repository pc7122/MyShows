...
<body onload="defaultTab('couponCode')">
    ...
    <section id="home" class="services">
        ...
        <div class="line" id="line">
            <h3>
                <marquee id="sd" Scrollamount=15> <?php echo getline(); ?> </marquee>
                <script>
                if (document.getElementById('sd').innerText == "") {
    ...
    <section id="advanced" class="services">
        ...
        <div class="tab">
            ...
            <button class="tablinks" onclick="openTab(event, 'multiplexStatus')">Multiplex Status</button>
        </div>
        ...
        <div id="multiplexStatus" class="tabs"></div>
...
function getline() {
    $d = "";
    $status = getmultiplexstatus();
    if($status == "CLOSE"){
        $d = "Multiplexes are currently closed ...";
    }
    else{
        $con=mysqli_connect("localhost","root","","myshows");
        ...
        if($data = mysqli_fetch_assoc($res)) {
            if(empty($data['state'])) {
                ...
                if($data['state'] == "ON") {
                    if($data['expiry']<date('Y-m-d')){
                        $query = "update socialdistance set state = 'OFF', date = '', expiry = ''";
                        $res=mysqli_query($con,$query);
                    }
                    else{
                    if($start < $today) {
                        $d = "Social Distancing Applied From ". $start ." till ". $expiry .".";
                    } else {
                        $d = "Social Distancing Applying  From ". $start .".";
                    }
                }
                } else {
                    $d = "";
...
function getmultiplexstatus(){
    $con = mysqli_connect("localhost", "root", "", "myshows");
    $query = "select * from multiplexstatus limit 1";
    $res = mysqli_query($con, $query);
    ...
    if($data = mysqli_fetch_assoc($res)) {
        if(empty($data['state'])) {
            ...
            if($data['state'] == "CLOSE") {
                if($expiry < $today) {
                    $query = "update socialdistance set state = 'OPEN'";
                    $res=mysqli_query($con,$query);
                    $d = "OPEN";
                }
                else {
