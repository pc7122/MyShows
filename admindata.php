...
elseif ($str[0] == 'movieUpdate') 
    {
        ...
        if(!empty($i)){
            ...
            if (uploadfile()) {
                $i = "assets/img/" . $i;
                $con = mysqli_connect("localhost", "root", "", "myshows");
                $query = "update movies set image = '$i' where moviename='" . $str[2] . "'";
                mysqli_query($con, $query);
            }
            else 
        ...
        if(!empty($wp)){
            ...
            if (uploadfile2()) {
                $wp = "assets/img/" . $wp;
                $con = mysqli_connect("localhost", "root", "", "myshows");
                $query = "update movies set wideposter='$wp' where moviename='" . $str[2] . "'";
                mysqli_query($con, $query);
            }
            else {echo "<script>alert('Sorry, your Image was not uploaded');window.location.href='admin.php#latestmovies';</script>";}
        }

        // echo "<script>alert('".$sc1."'); </script>";

        $con = mysqli_connect("localhost", "root", "", "myshows");
        $query = "update moviedetails set moviename = '$n',language = '$l',releasedate = '$r',cast = '$c', music = '$m', producer = '$pr',director = '$di',description = '$de' where moviename='" . $str[2] . "'";
        mysqli_query($con, $query);

        $con = mysqli_connect("localhost", "root", "", "myshows");
        $query = "update movies set moviename = '$n',genre = '$g',trailer = '$t',price = '$p'
        ,screen1 = '$sc1',screen2 = '$sc2',screen3 = '$sc3', type = '".$str[1]."',expiry='$e', releasedate ='$r',viewtype = '$vt' where moviename='" . $str[2] . "'";
        mysqli_query($con, $query);
        // echo "update movies set moviename = '$n',genre = '$g',trailer = '$t',price = '$p',screen1 = '$sc1',screen2 = '$sc2',screen3 = '$sc3', type = '".$str[1]."',expiry='$e', releasedate ='$r',viewtype = '$vt' where moviename='" . $str[2] . "'";
...
elseif ($str[0] == 'applycoupon') {
    ...
    // echo $state.$date;
    $con = mysqli_connect("localhost", "root", "", "myshows");
    $query = "update socialdistance set state='$state', date='$date', expiry='$expiry'";
    mysqli_query($con, $query);
    echo ("values are updated");
} elseif ($str[0] == 'setmultiplexstatus') {
    $state = $_POST['state'];
    $date = $_POST['date'];
    $expiry = $_POST['exp'];
    ...
    // echo $state.$date;
    $con = mysqli_connect("localhost", "root", "", "myshows");
    $query = "update multiplexstatus set state='$state', date='$date', expiry='$expiry'";
    mysqli_query($con, $query);
    echo ("values are updated");
}
...
elseif($str[0] == 'getmultiplexstatus') {

    $con=mysqli_connect("localhost","root","","myshows");
    $query = "select * from multiplexstatus";
    $res = mysqli_query($con, $query);
    $d="";
    ...
    if ($data = mysqli_fetch_array($res)) {
        ...
        $d .= '
            <form onsubmit="event.preventDefault()" action="" method="POST" name="socialDistancingForm" class="socialDistancingForm" id="MultiplexStatus">
                            
                <label>Multiplex Status</label>

                <label class="switch">
                    <input class="switch-input" name= "state" type="checkbox" />
                    <span class="switch-label" data-on="Close" data-off="Open"></span> 
                    <span class="switch-handle"></span> 
                </label>
                
                <label for="social">Select Closing Date</label>
                <input type="date" name="social" id="start" value="' . $date . '"  >
                
                <label for="exp">Select Opening Date</label>
                <input type="date" name="exp" id="exp" value="' . $expiry . '" >
            ...
            <script>
                ...
                $("#MultiplexStatus button").click(function () {
                    let state;
                    if ($("#MultiplexStatus .switch-input:checked").length == 1) {
                        state = "CLOSE";
                    }
                    else {
                        state = "OPEN";
                    ...
                    $.ajax({
                        ...
                        type: "post",
                        data: {
                            str: "setmultiplexstatus",
                            state: state,
                            date: $("#MultiplexStatus #start").val(),
                            exp:$("#MultiplexStatus #exp").val()
                        },
                        success: function (data) {
                            alert(data);
                        }
